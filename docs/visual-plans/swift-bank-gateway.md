...
Parallel prototype — Phase A complete, Phase A.1 preview wiring complete: new route-map visual (`frontend/src/viz/swiftRoleMap.ts`) created alongside the chip-board visual. Role 1 (Channel and secure access) implemented with BRM grammar: directed source→estate→service-path→destination layout, boundary gates, function core, evidence band, settlement block structurally outside estate. Phase B1 complete: Role 2 (Scheme connector) enabled. Roles 3–6 remain disabled. Wired into render.ts and types.ts as `swift-role-map-preview` block kind — prototype preview block appears below the chip-board visual, labelled "Prototype preview — SWIFT role-route map". Not yet the live teaching block; chip-board remains live above it.
