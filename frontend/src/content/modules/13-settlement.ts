...
const content: ModuleContent = {
  ...
  blocks: [
    ...
    },
    {
      ...
      body: "Gross settlement (RTGS) settles each transaction individually in real time. It is the safest approach, since once a payment is processed it is final and irrevocable, but it is also capital-intensive, because you need full liquidity for every single payment at the moment it is made. Netting takes the opposite approach. It aggregates many transactions between participants and settles only the net difference. If Bank A owes Bank B $100M and Bank B owes Bank A $80M, netting settles a single $20M transfer instead of two gross payments. This is far more capital-efficient but introduces delay and counterparty risk during the netting window. Until final settlement, every participant is exposed to every other participant defaulting. Most wholesale systems use a hybrid: netting during the day, with final gross settlement at end-of-day or at specific cutoff times. Fedwire in the US is pure RTGS, while the CHIPS system nets throughout the day and settles gross at close. Crypto generally settles gross, with every transaction individually final on-chain, which is one reason DeFi is so capital-hungry. There is no netting layer to reduce the liquidity each participant must hold.",
    },
    {
      kind: 'flow',
      ...
      data: {
        ...
        caption: "Delivery-versus-payment (DvP) on a single shared ledger makes both legs part of one atomic transaction. If either leg would fail, the whole transaction reverts, so neither party is ever exposed to \"I paid but didn’t receive.\"",
      },
    },
    {
      kind: 'text',
