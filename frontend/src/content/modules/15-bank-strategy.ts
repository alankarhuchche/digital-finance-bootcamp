  ...
  blocks: [
    ...
    },
    {
      ...
      data: {
        ...
        items: [
          { id: 'build', label: 'Build your own token', color: '#7AA7D9', values: [`Full control, keeps the deposit on balance sheet (tokenized deposit)`, `Significant build cost; only works if customers actually adopt it over USDC/USDT`, `JPMorgan Kinexys ($1.5B+ daily settlements); Société Générale FORGE (digital bonds issued on Ethereum public chain)`] },
          { id: 'partner', label: 'Offer customer access to an existing issuer', color: '#E8A33D', values: [`Keeps the customer relationship and fee income even as the deposit leaves`, `Doesn't defend the balance sheet at all — pure retention play`, `Standard Chartered's Zodia Markets (institutional crypto trading); multiple banks embedding USDC purchase in mobile apps`] },
          { id: 'consortium', label: 'Join a bank consortium', color: '#C792E8', values: [`Shares build cost; collective scale against Big Tech/crypto-native rivals`, `Slower, consensus-driven; depends on every member actually adopting it`, `Qivalis (EU banks), Regulated Liability Network (RLN)`] },
...
Banks are hedging. But what happens when things go wrong? The next topic catalogues the failure modes — the six ways digital finance systems actually break.`,
    },
    {
      ...
      data: {
        title: 'JPMorgan Kinexys: from Onyx to $1.5B+ daily settlements',
        dateRange: '2020–present',
        whatHappened: `JPMorgan launched its blockchain-based payment platform as "Onyx" in October 2020, initially processing a modest volume of intrabank transfers using JPM Coin — a permissioned token representing dollar deposits held at JPMorgan. By 2023, the platform was processing over $1 billion per day. In late 2024, JPMorgan rebranded Onyx to "Kinexys" and expanded beyond payments into multi-currency settlement and cross-border treasury management. As of early 2025, Kinexys processes $1.5B+ in daily settlement volume, with clients including Siemens, FedEx, and Cargill using it primarily for 24/7 cross-border treasury transfers — settling in minutes what previously took 1–3 business days through correspondent banking. Kinexys runs on a private, permissioned blockchain (based on a modified Ethereum codebase) with JPMorgan as the sole validator. Every token is fully backed by dollar deposits at JPMorgan — it's a tokenized deposit, not a stablecoin, which means it never leaves the bank's balance sheet.`,
        whyItMatters: `Kinexys demonstrates the "build" strategy at scale. By keeping everything on-balance-sheet as tokenized deposits rather than issuing stablecoins, JPMorgan gets the speed and programmability benefits of blockchain settlement without cannibalizing its own deposit base. The tradeoff is that Kinexys only works for JPMorgan clients moving money between JPMorgan accounts — it's a closed-loop system, powerful within its walls but not interoperable with the broader crypto ecosystem. Whether that's a feature or a limitation depends on whether you think the future is open or permissioned networks.`,
